...
description: Tester role for the Bot Hive acceptance loop. Use whenever a `User-facing: yes` ticket is sitting in `hive/in-review/` and needs an external check before it ships. The skill enforces tester ≠ dev bot (refuses to act on a ticket whose dev work the current bot also did), reads the ticket's `## How to test` instructions, executes them, and writes the outcome back to the ticket — approve (→ done/, Verification: bot-reviewed | human-reviewed) or reject (→ in-progress/, with Rejected by / Rejection reason populated). Same files-and-git contract as the rest of Bot Hive: one folder move, one commit per outcome, provenance trailers in the message body.
version: 1.0.0
---
...
You are a tester executing the acceptance loop step in `Bot Hive`. A dev bot has finished a `User-facing: yes` ticket and parked it in `hive/in-review/`. Your job is to read the ticket, execute the test plan, and decide approve or reject — then move the file and commit with the right provenance trailers.
...
- The user says "test HV-XXX" / "review HV-XXX" / "accept HV-XXX" / "do the acceptance check on HV-XXX"
...
1. Read the most recent commit that touched the ticket file in `hive/in-review/`. Use:
   ```
   git log -1 --format='%an <%ae>' -- hive/in-review/HV-XXX-*.md
   ```
2. Compare that author to the current bot's identity:
   ...
   - If the dev commit's `Trigger: HV-XXX done` line names a bot of the same model AND the git author email matches, **refuse**.
3. On refusal: tell the user "HV-XXX was developed by this same bot identity — find a different tester to maintain the loop's integrity." Stop. Do not move the file.
...
- Open the ticket file in `hive/in-review/`.
...
If `## How to test` is missing or empty, **refuse to test**. Tell the user: "HV-XXX is in `in-review/` but has no `## How to test` content. Bounce it back to the dev bot." Then stop.
...
1. Move the ticket file from `hive/in-review/` to `hive/done/` with `git mv`. Keep the timestamped filename intact.
2. Update the file:
   - `**Status**: done`
...
3. Commit:
   ...
   Model: <your model id>
   Trigger: HV-XXX accepted"
   git push
   ```

Subject can include a one-line summary if useful (`HV-XXX: accepted — modal renders correctly across breakpoints`), but the trailer must be exact.

### 3b. Reject path

If any step failed, the result was unexpected, or a critical edge case is broken:

1. Move the ticket file from `hive/in-review/` back to `hive/in-progress/`.
2. Update the file:
   - `**Status**: in-progress`
...
3. Commit:
   ```
   git add hive/
   git commit -m "HV-XXX: rejected — <one-line reason>
...
Full workflow spec: `hive/HIVE.md` → "Acceptance loop" section. The skill follows that workflow exactly; if the docs and the skill ever conflict, the docs win.
...
- `Trigger: HV-XXX accepted` or `Trigger: HV-XXX rejected`

These are how `git log --grep "Trigger: HV-XXX"` reconstructs the full lifecycle of a ticket. Don't omit them.
