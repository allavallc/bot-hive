...
> This file describes how Bot Hive's *own* development is coordinated using the bot-horde ticket format. Bot Hive (the SaaS product) reads `hive/` folders just like this one in user repos. The format here is the operational dogfood — we build Bot Hive against the same kanban it ships.
>
> **Note:** This doc was adapted from bot-horde's `BOTHORDE.md` (see `plan/bot-horde-spec.md` for the unmodified reference). Sections describing static board rebuilding via GitHub Actions / `build.sh` / `validate.py` will not apply once Bot Hive's Next.js board (HV-007) is live — the SaaS *is* the board renderer. Until then, the hive/ folder is read-only markdown.

---

## Scope

> **Bot Hive is local developer and bot tooling. It is not product code.** Tickets, the generated board, the builder script, and the config file are coordination artifacts between developers and agents working on a repo. **Bot Hive files must never enter a deploy artifact.** If you see Bot Hive files inside a Docker image, a build output, or a running production service, that is a bug — not an install step.

**One-way dependency.** Bot Hive knows about your code — tickets can reference files, endpoints, and features. Your code must not know about bot-hive. If product code imports anything under `bot-hive/`, references a ticket ID in a runtime path, or relies on `HIVE.md` existing at build time, that is a bug.

---

## Out of scope

Bot Hive is a coordination layer over files-and-git. The following are explicitly **out of scope** and will not be added to this project:
...
- **Semantic-similarity deduplication, budget enforcement, cross-repo coordination.** Out of scope; Bot Hive is single-repo, single-tree, and trusts adopters to manage their own bot economics.

If you need any of the above, build it as a **layer on top** — reading the ticket files — not by extending Bot Hive itself. Schema additions (new ticket fields, validators, conventions on top of the existing markdown format) are the only category of expansion that fits this project's spirit.

---

## Sync modes

**Git sync (default).** Tickets live in the repo. `git pull` before reading the backlog, `git push` after moving a ticket. All agents and developers see the same board. `setup.sh` installs `.github/workflows/bot-hive-rebuild-board.yml`, which rebuilds the board on every push to `main`.

**Local-only.** Add `bot-hive/` to `.gitignore`. The board still renders for a single machine, but collaboration is disabled — each developer's tickets are private to their own working tree. Use only if developers manage their own work independently. **The rebuild-board workflow is intentionally not installed in this mode** (the workflow runs in CI where `bot-hive/build.sh` isn't present, so it would fail every push). Re-run `setup.sh` and it will detect the mode and prompt to remove a stale workflow if one is present.

---

## For LLMs reading this file

- Treat Bot Hive files as coordination metadata, not code. Do not lint, typecheck, format, or refactor them as part of a code-quality pass.
- When summarizing repo changes, exclude Bot Hive file changes by default unless the user explicitly asks about workflow.
- When writing tests, CI config, or deploy scripts, assume `bot-hive/` does not exist at build time.
- Never wire a Bot Hive ticket ID (e.g. `// see HV-042`) into product source. Ticket IDs must not survive into shipped code.

---

## How to update Bot Hive

Run this check whenever the user asks "is Bot Hive up to date?", at the start of a session, or before drafting a new ticket:

```bash
HIVE_CHECK_UPDATES=yes bash <(curl -fsSL https://raw.githubusercontent.com/allavallc/bot-hive/main/setup.sh)
```

**Decision logic:**

- **Exit code 0** → already current. Continue with the user's task.
- **Exit code 1** → at least one file is stale. The stdout lists which. **Surface the list to the user. Do not auto-upgrade — ask first.** Example phrasing: "Bot Hive has updates: HIVE.md (2026-04-27 → 2026-05-01). Run upgrade now? (y/n)"

If the user approves, run:

```bash
HIVE_UPGRADE_DOCS=yes bash <(curl -fsSL https://raw.githubusercontent.com/allavallc/bot-hive/main/setup.sh)
```

Then commit the result:

```bash
git add bot-hive/ .github/workflows/bot-hive-rebuild-board.yml
git commit -m "chore: update Bot Hive"
git push
```
...
- Files in the managed list (see `bot-hive/MANIFEST.json` `files` map) that you've manually edited will be overwritten by upgrade. The agent must not let the user upgrade without first surfacing this if the user has local edits.
- For non-interactive / CI use: `HIVE_AUTO_MIGRATE=yes`, `HIVE_UPGRADE_SKILL=yes`, and `HIVE_UPGRADE_DOCS=yes` env vars bypass all prompts.
- `bot-hive/UPDATING.md` is a copy of this section as a standalone file — useful when the user wants the docs without loading the whole HIVE.md.
...
Tickets live as individual markdown files in the `bot-hive/` folder in this repo. Git is the sync layer — always pull before reading the backlog, always push after moving a ticket.
...
1. `git pull` silently
2. Read every file in `bot-hive/backlog/`, `bot-hive/in-progress/`, and `bot-hive/feature-sets/`
...
|---|---|---|
| HV-071 | Split-from field | Records lineage when one ticket becomes many. |
| HV-072 | Verification field | Trust level separate from Status. |
...
1. **Read context before drafting.** Before writing a single line of the ticket:
   - Scan `bot-hive/backlog/` and `bot-hive/in-progress/` for related or duplicate tickets
   - Scan `bot-hive/feature-sets/` for an existing feature set that fits
...
3. **Decide feature-set membership.** Either match an existing `feature-set-XXX.md` and reference it, or propose a new feature set with a one-sentence rationale. New feature sets get scaffolded automatically into `bot-hive/feature-sets/`.
...
1. Create the ticket file in `bot-hive/backlog/HV-XXX-<timestamp>.md` using the ticket format below
2. If a new feature set was proposed, create `bot-hive/feature-sets/feature-set-XXX-<slug>.md`
...
There is no separate lock registry. The git push *is* the lock: the agent that successfully moves a ticket file to `bot-hive/in-progress/` and pushes wins. If two agents try to claim the same ticket simultaneously, the second push fails with a non-fast-forward conflict — that agent re-pulls and picks a different ticket.
...
- `in-review` — dev bot handed the ticket off to `bot-hive/in-review/` for tester signoff.
...
1. **Check `Blocked by` first.** If the ticket lists any IDs in `**Blocked by**`, verify each is in `bot-hive/done/`. If any blocker is unfinished, do not claim — surface the dependency to the user instead. (The board's "Ready only" filter does this same check on the rendering side; the validator in HV-076 enforces it at done-transition time.)
2. Move the file from `bot-hive/backlog/` to `bot-hive/in-progress/` (keep the full filename including timestamp)
...
When a ticket cannot proceed, move it to `bot-hive/blocked/` and set both the `**Blocked by**` field (which HV-IDs are preventing progress) and the `**Failure mode**` field (the *category* of blocker). Both are **required** for tickets in the blocked folder — a blocked ticket with no explanation of what's blocking it is just an orphan.
...
2. Move the file from its current folder to `bot-hive/blocked/`
3. Run:
   ```
   git add bot-hive/
...
3. Move the ticket file to `bot-hive/not-doing/`
...
3. **Move each source ticket to `bot-hive/not-doing/`** with `**Rejection reason**: consolidated into HV-XXX` (substitute the new ticket's ID).
...
2. Move the ticket file from `bot-hive/in-progress/` to `bot-hive/done/`
...
2. Move the ticket file from `bot-hive/in-progress/` to `bot-hive/in-review/`
...
CI runs `python3 bot-hive/validate.py` on every push (via `build.sh`). The validator walks every ticket and checks:
...
- A `done/` ticket cannot have `Blocked by` references that aren't themselves `done/` (HV-075 enforcement)
...
**Write only one side of each edge.** If HV-005 declares `Blocks: HV-006`, do not also add `Blocked by: HV-005` on HV-006 — the renderer infers the inverse at build time and shows it on the counterpart card automatically. Writing both sides creates maintenance drift.
...
Read all files across all folders in `bot-hive/` including `not-doing/`. Find the highest existing HV-XXX number and increment by 1. Start at HV-001 if no tickets exist.
...
- Feature set files live in `bot-hive/feature-sets/` (named `feature-set-XXX-<slug>.md`)
...
The board updates from git, full stop. Whenever an agent pushes a ticket move, the GitHub Actions workflow rebuilds `bot-hive/board.html`. Browsers viewing the board poll the GitHub commits API every ~30 seconds; if the commit SHA on `main` changes, the page reloads. There is no broadcast layer, no WebSocket, no database. Push-to-board latency is bounded by the polling interval.

**Polling is GitHub-specific.** Live updates work on GitHub-hosted repos (via the GitHub commits API). For adopters running the board off-platform — GitLab, Gitea, self-hosted git, or a static export — set `"pollSource": "none"` in `bot-hive/config.json`. The masthead then shows `static — refresh manually` instead of pretending the page is live. Reads (Add Story) still go through the GitHub Contents API, so non-GitHub adopters use the board read-only.

---

## Board rules

- The board is rebuilt automatically by GitHub Actions on every push to `main` — never run `bot-hive/build.sh` manually
- Feature set files are named `feature-set-XXX-<slug>.md` and live in `bot-hive/feature-sets/`
- Never edit `bot-hive/board.html` directly — it is always generated by `bot-hive/build.sh`
...
The board-rebuild workflow always **builds** the board on every push (to surface build failures in CI) but only **commits** the result when configured to. The behavior is controlled by the `auto_commit_board` field in `bot-hive/config.json`:

- `"auto_commit_board": true` (default) — workflow commits `bot-hive/board.html` back to `main` so GitHub Pages stays fresh.
