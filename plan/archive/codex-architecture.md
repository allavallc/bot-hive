...
|---|---|---|---|
| Agent-neutral architecture definition | `plan/codex-architecture.md` | Done | North-star architecture captured here. |
...
| Swarm panel live event rendering | `HV-148` | Next / open | Add `bot_events` to project events API and render them in the swarm panel. |
| Tester approval merge gate | `HV-149` | Open | Make tester approval explicit and enforceable before merge-ready state. |
| Server-side context injection | `HV-150` | Open | Inject feature-set/ticket/learning context into task and review handoff events. |
...
- `npm.cmd run test` passed: 14 files, 140 tests.
...
- `role-changed`: a bot left or joined and this bot's role changed
